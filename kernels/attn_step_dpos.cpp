// Decode-step GQA attention (parametric pos — total_len read from a
// device buffer so the forward is HIP-graph-capturable).
                          ...
                          float        scaling)
{
    ...
    const unsigned int total_len = *pos_ptr + 1;  // decode: cache len = pos+1
