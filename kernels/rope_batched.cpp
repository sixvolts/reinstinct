...
// Variant that reads `base_pos` from a device-resident uint32 so it
// can be updated host-side between captured-graph replays. See the
// comment in kv_quant_prefill_offset_f32.
extern "C" __global__
void rope_apply_batched_offset_f32(float*       __restrict__ x,
                                   ...
                                   const unsigned int* __restrict__ base_pos_ptr)
{
    ...
    const unsigned int pos = (*base_pos_ptr) + row;
    float* head     = x + ((size_t)row * n_heads + h) * head_dim;
    const float* cr = cos + (size_t)pos * rotary_dim;
    const float* sr = sin + (size_t)pos * rotary_dim;
