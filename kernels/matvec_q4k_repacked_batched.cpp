...
void mv_q4k_batched_impl(const uint8_t* __restrict__ wbase,
...
void matvec_q4k_repacked_batched_f32(const uint8_t* __restrict__ wbase,
                                     ...
                                     unsigned int n_rows)
{
    mv_q4k_batched_impl<2, 4>(wbase, xq, y, in_dim, out_dim, n_rows);
}
...
extern "C" __global__
void matvec_q4k_repacked_batched16_f32(const uint8_t* __restrict__ wbase,
                                     const BlockQ8* __restrict__ xq,
                                     float*         __restrict__ y,
                                     unsigned int in_dim,
                                     unsigned int out_dim,
                                     unsigned int n_rows)
{
    mv_q4k_batched_impl<1, 16>(wbase, xq, y, in_dim, out_dim, n_rows);
}
